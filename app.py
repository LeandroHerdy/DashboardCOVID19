...
brazil_states = json.load(open("geo/brazil_geo.json", "r"))
...
app.layout = dbc.Container(
    dbc.Row([       
        ...
        dbc.Col([
            html.Div([
                html.Img(id="logo", src=app.get_asset_url("logo_dark.pmg"), height=50),
        ...
        dbc.Row([
            ...
            dcc.Dropdown(id="location-dropdown",
                         options=[{"label": i, "value": i} for j, i in select_columns.items()],
                         value="casosNovos",
                         style={"margin-top": "10px"}   
                        ),   
            dcc.Graph(id="line-graph-map", figure=fig2)        
            ]),
            
        ], md=5, style={"padding": "25px", "backgroud-color":"#242424"}),
...
@app.callback(Output("line-graph", "figure"),
            [
              Input("location-dropdow", "value"), Input("location-button", "children"),  
            ]
            )
def plot_line_graph(plot_type, location):
    ...
    bar_plots = ["caasosNovos", "obitosNovos"]
    fig2 = go.Figure(layout={"template": "plotly_dark"})
...
@app.callback(
    ...
    [Input("choropleth-map", "chickData"), Input("location-button", "n_clicks")]
)
